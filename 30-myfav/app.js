...
maplibregl.addProtocol('custom', (params, callback) => {
  ...
  const imgs = [
    ...
    `https://tiles.gsj.jp/tiles/elev/gsidemgm/${z}/${y+1}/${x}.png`
  ]
  const imgd = []
  ...
  loader(imgs, loadImage, ()=>{
    ...
    const gj = {
      ...
      "features": [...Array(n+Math.floor(n/4))].map((a, i)=>{
        return {
          "type": "Feature",
          "properties": {'color': `rgb(128, 128, ${Math.abs((lat+90)*255/180-i*r/Math.pow(2,z-1)-128)+128})`},
          "geometry": {
            "coordinates": 
            [...Array(r*n)].map((b, j)=>[
              lon+(j+j/(r*n))*coe/(r*n),
  ...
  style: {
    ...
    "sources": {
      "gsi": {
        ...
        "minzoom": 0,
        "maxzoom": 8,
        "attribution": "<a href=\"https://gbank.gsj.jp/seamless/elev/\" target=\"_blank\">DEM by 産業技術総合研究所</a>"
      }
    },
    "layers": [
      {
